...
targetfeat = np.load("bladefeats.npz")
...
num_iter =30000
target=np.load("church-3255_init.npy")
...
for i in range(st_iter+500,st_iter+num_iter):
    ...
    for j in range(5):
        ...
        if i%500==0:
            ...
            imsave("churchtocleaver/samp_%d_%d.png"%(i,j),imgno[j][0].astype(np.uint8))
            np.save("churchtocleaver/para_%d_%d.npy"%(i,j),parano[j])
