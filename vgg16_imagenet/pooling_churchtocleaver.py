...
class vgg16:
    ...
    def lossandgrad(self,featfc1,featfc1_s,featfc1_relu,img1,upperbound):
        ...
        self.posloss = tf.nn.l2_loss(tf.boolean_mask(featfc1,featfc1_s)-tf.boolean_mask(self.conv5_3,featfc1_s))
        tmp = (featfc1_s-1).nonzero()[1]
        neg_mask = np.logical_not(featfc1_s)
        self.negloss=[]
        self.loss_am=[]
        self.grad = []
        ids = [49202, 55978, 48185 , 1780, 72388]
        upb = [ 79.22801971,  45.25467682 , 30.46495819,  29.10372734,  26.54627991]
        for i in range(5):
            self.negloss.append(tf.abs(tf.boolean_mask(self.conv5_3,neg_mask)[ids[i]]-upb[i])
                           +tf.reduce_sum(tf.abs(tf.boolean_mask(self.conv5_3,neg_mask)[:ids[i]]))
                            +tf.reduce_sum(tf.abs(tf.boolean_mask(self.conv5_3,neg_mask)[ids[i]+1:])))
            self.loss_am.append(100*self.posloss+10*self.negloss[i]+100*self.diffnorm)
...
vgg = vgg16(paras,'../imgnet_32/imagenet_n03028079_3255_overlap.npz', 'vgg16_weights.npz', sess)
...
targetfeat = np.load("bladefeats_pool.npz")
featconv5 = targetfeat["featconv5"]
featpool5 = targetfeat["featpool5"]
mask = targetfeat["mask"]
vgg.lossandgrad(featconv5,mask,featpool5,img1,targetfeat["upperbound"])
...
num_iter =30000
target=np.load("init_raw_overlap.npy")
...
for i in range(5):
    ...
    #parano.append(np.load("results_knn2/para_%d_%d.npy"%(st_iter,i)))
    realgrad.append(0.0)
...
for i in range(st_iter,st_iter+num_iter):
    ...
    for j in range(5):
        ...
        if i%500==0:
            ...
            imsave("churchtocleaver_pool/samp_%d_%d.png"%(i,j),imgno[j][0].astype(np.uint8))
            np.save("churchtocleaver_pool/para_%d_%d.npy"%(i,j),parano[j])
