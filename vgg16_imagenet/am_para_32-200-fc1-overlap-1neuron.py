...
class vgg16:
    ...
    def lossandgrad(self,featfc1,featfc1_s,featfc1_relu,img1):
        ...
        for i in range(5):
            ...
            self.loss_am.append(100*self.posloss+1*self.negloss[i]+100*self.diffnorm)
...
vgg = vgg16(paras,'../imgnet_32/imagenet_n02086646_1410_edit.npz', 'vgg16_weights.npz', sess)
...
targetfeat = np.load("fishfeats.npz")
...
num_iter = 1000000
lr = 0.001#0.00000001
target=np.load("init_raw_4_overlap.npy")
...
for i in range(5):
    ...
    parano.append(np.load("results_knn/para_%d_%d.npy"%(st_iter,i)))
    realgrad.append(0.0)
...
for i in range(st_iter+1000,st_iter+num_iter):
    if i%1000==0:
        print "\n"
        print i
    for j in range(5):
        ...
        if i%1000==0:
            l1,neg1,imgno[j] = sess.run([vgg.posloss,vgg.negloss[j],vgg.imgs], feed_dict={vgg.paras: parano[j]})
            imsave("results_knn/samp_%d_%d.png"%(i,j),imgno[j][0].astype(np.uint8))
            np.save("results_knn/para_%d_%d.npy"%(i,j),parano[j])
