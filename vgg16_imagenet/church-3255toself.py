...
targetfeat = np.load("church3255feats.npz")
...
for i in range(5):
    ...
    parano.append(np.load("%s/para_%d_%d.npy"%(foldername,st_iter,i)))
    realgrad.append(0.0)
...
for i in range(st_iter+500,st_iter+num_iter):
    if i%500==0:
        print i
    if i==2200: lr=0.01
    if i==3200: lr=0.02
    ...
    for j in range(5):
        ...
        if i%500==0:
            ...
            imsave("%s/samp_%d_%d.png"%(foldername,i,j),imgno[j][0].astype(np.uint8))
            np.save("%s/para_%d_%d.npy"%(foldername,i,j),parano[j])
